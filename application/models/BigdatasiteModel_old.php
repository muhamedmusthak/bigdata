...
function associate($id)
    ...
    //$this->db->join('class_enroll', 'class_enroll.enroll_id = enroll.enroll_id');
       ...
       return $data;
		...
		$data=array(
			...
			'contact'=>$this->input->post('phone_num'),
   ...
   // 		{	
   // 			// print_r($post);
   // 			// print_r(count($post)."asdasd");
   // 			// print_r(sizeof($post)."size of");
   // 			$conut =count($post);
	...
	function ins()
	{
		$data=array(
		  ...
		  return $data;
			...
			// "SELECT * FROM `assessment_questions` WHERE course_name='asscociate-bigdata'";
// print_r($value['cid']);exit();
			// 	$array = array('course_id' => $value['cid'], 'pattern' => $value['cpattern']);
			// $this->db->select('*');
		 //    $this->db->from('assess_question');
		 //    $this->db->where($array);

				// $cid=$value['cid'];
				// "SELECT course_id FROM enroll WHERE enroll_id='$value'";
				// $pattern=$value['cpattern'];
   ...
   //  			'result' => $result,
	...
	// 		echo '<pre>';count($this->input->post())
			//print_r($this->input->post());exit();
	 // print_r($this->input->post('quesId1'));exit;
